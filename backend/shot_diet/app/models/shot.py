...
class Shot(db.Model):
    ...
    player_shot_team_year_id = db.Column(db.Integer, db.ForeignKey('player_shot_team_year.id'))
    ...
    player_shot_team_year = db.relationship('PlayerShotTeamYear', backref='shots', lazy=True)
    ...
    def combine_to_dataframe(cls, shots):
        ...
        shot_data = [shot.get_data() for shot in shots]  # Modify to return a dictionary of shot data
    ...
    def expected_ts_df(cls, shots, season, team_id=None):
        ...
        groupby_cols = ['PLAYER_ID', 'ACTION_TYPE', 'SHOT_TYPE', 'shot_value', 'SHOT_ZONE_BASIC', 'SHOT_ZONE_RANGE', 'SHOT_ZONE_AREA']
        player_ids = df['PLAYER_ID'].unique()
        for player in player_ids:
            player_shot_data = df[df.PLAYER_ID == player]
            if team_id:
                season_player = player_shot_data[player_shot_data['season_year'] == season]
                season_player = season_player[season_player['TEAM_ID'] == team_id]
            else:
                season_player = player_shot_data[player_shot_data['season_year'] == season]

            past_shots = Shot.query.filter(Shot.PLAYER_ID == int(player), Shot.season_year < str(season)).all()
            if not past_shots:
                continue
            ...
            all_at_once = training_data.groupby(groupby_cols)['SHOT_MADE_FLAG'].agg(['mean', 'sum', 'count']).reset_index().rename(
                columns={'mean': 'EXPECTED_FG_PCT', 'sum': 'TOTAL_FEATURE_MATCHES_MADE', 'count': 'NUM_FEATURE_MATCHES'}
                )
            all_at_once['EXPECTED_TS'] = all_at_once['EXPECTED_FG_PCT'] * all_at_once['shot_value']
